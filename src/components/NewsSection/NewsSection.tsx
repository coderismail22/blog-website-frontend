    ...
    <div className="md:w-10/12 mx-auto border-t-4 border-black border-b-4 pb-10">
      ...
      <div className={`md:flex ${flexReverse ? "flex-row-reverse" : ""}`}>
        ...
        <div className={`md:w-[70%] ${flexReverse ? "border-l md:pl-4" : "border-r md:pr-4"}`}>
          <div>
            <img src={post.coverImage} alt="" className="w-full border h-[50vh]" />
            <div className="py-5 space-y-2">
              ...
              <p className="text-center text-[12px] font-bold">By {post.author}</p>
            </div>
          </div>
                ...
                <div className="flex flex-row-reverse gap-1 pb-4 mt-5">
                  <div className="w-[70%]">
                    <p className="font-Playfair text-[15px] font-semibold">
                      <Link to={`/news-details/${related.slug}`}>
                        {related.title}
                      </Link>
                    </p>
                  </div>
                  <div className="w-[30%]">
                    <img src={related.coverImage} className="w-full h-full" alt="" />
                  </div>
                </div>
              </div>
            ))}
          ...
          {post?.sidebarPosts?.slice(0,2)?.map((sidebar) => (
            <div key={sidebar._id} className="">
              ...
              <p className="font-Playfair font-bold">
                <Link to={`/news-details/${sidebar.slug}`}>
                  {sidebar.title}
                </Link>
              </p>
